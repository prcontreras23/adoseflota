          ...
          <div>
            ...
            {linea.titular_responsable && (
              <p className="text-xs text-slate-500 dark:text-slate-400 flex items-center gap-1 mt-0.5">
                ...
                Titular: <span className="font-semibold text-slate-600 dark:text-slate-300 ml-0.5">{linea.titular_responsable}</span>
              </p>
            )}
          </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex gap-2 flex-wrap">
          {(() => {
            ...
            return (
              <button
                onClick={() => allSelected ? setSelectedIds(new Set()) : setSelectedIds(new Set(allVisibleIds))}
                className="text-sm px-4 py-2 rounded-xl font-semibold transition-colors flex items-center gap-1.5 border bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 border-slate-200 dark:border-slate-600 hover:bg-slate-200 dark:hover:bg-slate-600"
              >
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="2"/>{allSelected && <polyline points="9 11 12 14 22 4"/>}</svg>
                {allSelected ? "Deseleccionar todo" : `Seleccionar todo (${allVisibleIds.length})`}
              </button>
            );
          })()}
          {totalArchivadas > 0 && (
                    ...
                    <p className="text-xs text-slate-400 flex flex-wrap items-center gap-x-2 gap-y-0.5">
                      ...
                      {tieneMontos && (
                        <span className="font-semibold text-emerald-600 dark:text-emerald-400 flex items-center gap-1">
                          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 000 7h5a3.5 3.5 0 010 7H6"/></svg> {fmtRD(montoGrupo)}
                        </span>
                      )}
