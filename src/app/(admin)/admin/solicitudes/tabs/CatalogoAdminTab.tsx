            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                    ...
                    <p className="text-sm text-slate-500 dark:text-slate-400">
                        ⭐ {dispositivos.filter(d => d.destacado).length}/6 dispositivos visibles para usuarios
                    </p>
                </div>
                <div className="flex gap-2">
                    <a href={CLARO_PDF_URL} target="_blank"
                        className="text-sm bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-400 px-4 py-2 rounded-xl font-medium hover:bg-red-100 transition-colors">
                        📄 Ver PDF Claro
                    </a>
                            ...
                            <div className="flex items-start justify-between gap-3">
                                <div className="flex-1">
                                    <div className="flex items-center gap-2 mb-1">
                                        ...
                                        {d.destacado && <span className="text-xs bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400 px-2 py-0.5 rounded-full font-semibold">⭐ Visible</span>}
                                ...
                                <div className="flex flex-col gap-1.5 items-end">
                                    <button onClick={() => toggleDestacado(d.id, d.destacado)}
                                        className={`text-xs px-2.5 py-1 rounded-lg font-medium transition-colors ${d.destacado ? "bg-yellow-100 text-yellow-700 hover:bg-yellow-200 dark:bg-yellow-900/30 dark:text-yellow-400" : "bg-slate-100 text-slate-500 hover:bg-slate-200 dark:bg-slate-700 dark:text-slate-400"}`}>
                                        {d.destacado ? "⭐ Quitar" : "☆ Destacar"}
                                    </button>
                                    <button onClick={() => setEditing(d)} className="text-xs bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-600 px-2.5 py-1 rounded-lg transition-colors">✏️ Editar</button>
                                    <button onClick={() => deleteDevice(d.id)} className="text-xs bg-red-50 dark:bg-red-950/30 text-red-500 hover:bg-red-100 dark:hover:bg-red-900/40 px-2.5 py-1 rounded-lg transition-colors">🗑️ Eliminar</button>
                                </div>
                            </div>
                        )}
                    </div>
